...
for script in scripts/diagnose-ieum-server.sh scripts/diagnose-ieum-external.sh scripts/ieum-cluster-tool.sh scripts/recover-ieum-node.sh; do
  bash -n "$script"
  output="$(bash "$script" -h)"
  grep -q '사용법' <<<"$output"
  grep -q 'v1.0.3.1' <<<"$output"
done

grep -q 'restart N' <<<"$(bash scripts/ieum-cluster-tool.sh -h)"
grep -q 'reproduce BIN' <<<"$(bash scripts/ieum-cluster-tool.sh -h)"
grep -q 'recover \[N\]' <<<"$(bash scripts/ieum-cluster-tool.sh -h)"
grep -q 'data/ledger' <<<"$(bash scripts/recover-ieum-node.sh -h)"
