...
nonce는 계좌별 거래 순번입니다. 계좌의 확정 nonce가 1이면 nonce 0 거래는 이미
처리된 거래이므로 다시 mempool에 들어가면 안 됩니다. P2P에서 늦게 재수신한 거래와
합의 timeout 뒤 복원되는 거래는 현재 체인의 확정 nonce·잔액·서명을 다시 검증하고,
